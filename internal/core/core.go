...
type PR struct {
	...
	// Head and Base are the PR's branches. A stack is derived from them and
	// never stored: B sits on A when B.Base == A.Head in the same repo.
	Head string `json:"head,omitempty"`
	Base string `json:"base,omitempty"`
	// HeadOID is the commit the PR's head branch points at on GitHub — compared
	// against a local worktree's HEAD to tell whether a local diff is current.
...
type Workspace struct {
	...
	// RetiredAt is when it was moved to trash — drives the Trash view's ordering
	// and its "time since close" column. Zero unless retired.
...
type Worktree struct {
	...
	Created time.Time // mtime of the worktree's `.git` file — a stable creation proxy
}
