...
func TestBranchName_ShellsToClaude(t *testing.T) {
	withPATH(t, writeStubClaude(t, "fix/login-bug"))
	got, err := New().BranchName("login broken on safari")
	if err != nil {
		t.Fatalf("BranchName: %v", err)
	}
	if got != "fix/login-bug" {
		t.Fatalf("BranchName: got %q", got)
	}
}

func TestSessionName_ShellsToClaude(t *testing.T) {
	withPATH(t, writeStubClaude(t, "investigate-flaky-tests"))
	got, err := New().SessionName("look into the flaky test on CI")
	if err != nil {
		t.Fatalf("SessionName: %v", err)
	}
	if got != "investigate-flaky-tests" {
		t.Fatalf("SessionName: got %q", got)
	}
}
