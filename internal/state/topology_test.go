...
func TestCaptureTopology_CapturesWindowCapability(t *testing.T) {
	...
	h.runOut["list-windows"] = strings.Join([]string{
		windowLine("$1", "@2", "1", "feat/x", "slug", liveDir, "/repo", "1", "1", "did a thing", "mytag", liveDir),
...
func TestCaptureTopology_RecapWithPipeSurvives(t *testing.T) {
	h := newFakeHost()
	h.setSessionsWithIDs("$1|vyrwu/atelier")
	// A recap containing '|' must not shift the fixed fields: @attention_recap
	// is emitted LAST and parsed with SplitN, so an embedded '|' is absorbed.
