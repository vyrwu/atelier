...
func handleCreateWorktree(raw json.RawMessage) map[string]interface{} {
	...
	// --no-track: branching from origin/<default> would otherwise make that the
	// new branch's upstream, and a plain `git push` from the agent (with
	// push.default=upstream) would land its work straight on the default branch.
	// create_pr sets the real upstream when it pushes.
	if out, err := runGit(source, "worktree", "add", "--no-track", "-b", a.Branch, target, "origin/"+def); err != nil {
...
func handleCreatePR(raw json.RawMessage) map[string]interface{} {
	...
	// Mid-rebase the worktree is on a detached HEAD, which would otherwise read
	// as "not on a branch" and hide what is actually wrong.
	if rebaseInProgress(path) {
		return textResult("a rebase is in progress in this worktree — resolve it, `git rebase --continue` "+
			"(or `git rebase --abort`), then call create_pr again", true)
	}
	...
	// Measure how far the base moved since we branched (before the rebase folds
	// it in) so we can warn the agent to re-verify — the branch is now current,
	// but decisions it made earlier may predate these commits.
	drift := 0
	if out, err := runGit(path, "rev-list", "--count", branch+"..origin/"+def); err == nil {
		drift, _ = strconv.Atoi(strings.TrimSpace(out))
	}
	if out, err := runGit(path, "rebase", "origin/"+def); err != nil {
		// A conflict leaves the rebase in progress, deliberately: aborting it
		// would leave nothing to resolve, and the next call would conflict the
		// same way. Anything else (a dirty tree, say) never started one.
		if rebaseInProgress(path) {
			return textResult("rebase onto origin/"+def+" stopped on conflicts and is left in progress. "+
				"Resolve them, `git add` the files, run `git rebase --continue`, then call create_pr again:\n"+out, true)
		}
		return textResult("rebase onto origin/"+def+" failed:\n"+out, true)
	}
	...
	msg := "opened PR: " + url
	if drift > 0 {
		msg += fmt.Sprintf("\n\n⚠ %s advanced %d commit(s) since you branched — re-verify your changes still hold against the current default.", def, drift)
	}
...
func runGit(dir string, args ...string) (string, error) {
	return runGitFor(0, dir, args...)
}

// gitNetTimeout bounds a git call that talks to the remote, so a stalled
// network fails the tool call instead of hanging it.
const gitNetTimeout = 2 * time.Minute

// runGitFor runs git in dir, cut off after d when d is set. It never asks for
// credentials on a terminal: the terminal here is the agent's, and a username
// prompt printed into Claude's UI hangs the tool call for good. With no
// credential to use, git fails and says so instead.
func runGitFor(d time.Duration, dir string, args ...string) (string, error) {
	...
	c.Env = append(os.Environ(), "GIT_TERMINAL_PROMPT=0", "GCM_INTERACTIVE=never")
	out, err := c.CombinedOutput()
	return strings.TrimSpace(string(out)), err
}

// rebaseInProgress reports whether a rebase is stopped part-way in the worktree
// at path (git keeps its state in rebase-merge or rebase-apply).
func rebaseInProgress(path string) bool {
	for _, dir := range []string{"rebase-merge", "rebase-apply"} {
		out, err := runGit(path, "rev-parse", "--git-path", dir)
		if err != nil {
			continue
		}
...
func gitAuthed(dir string, args ...string) (string, error) {
	...
	return runGitFor(gitNetTimeout, dir, append(pre, args...)...)
}
