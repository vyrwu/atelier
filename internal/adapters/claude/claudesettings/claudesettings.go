...
// atelier's required Stop hook (`atelier ai on-stop`, the kernel's AI
// stop-hook entry) without mutating the user's
// `~/.config/claude/settings.json` — Claude CLI layers our settings on top
// of the user's, so agent-deck and other user hooks continue to fire.
//
...
//
// v3: hook command moved from `atelier tools claude notify-attention` (the
// pre-hexagonal claude tool, now deleted) to `atelier ai on-stop`. The bump
// is load-bearing — without it, an upgrading user's cached v2 file keeps the
// dead command and Claude's attention/recap silently stop firing.
...
//     `atelier ai on-stop` command. Without this, selectors silently
//     stall Claude without flagging the parent window.
//
// on-stop is idempotent for the same window so wiring two
...
func canonical() canonicalSettings {
	notify := []hookGroup{
		{Hooks: []hookEntry{{
			Type:    "command",
			Command: dispatch.CoreCmd("ai", "on-stop"),
		}}},
	}
