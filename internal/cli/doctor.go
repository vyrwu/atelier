...
// segments survive in `window-status-current-format`. That's the
// canonical, always-stamped location for both segments: the bundled
// theme hides inactive windows (empty window-status-format), so the
// current-format is where the icons actually render. The bundled
// launcher stamps this on every startup, but a host-config statusline
// (plugin mode) can overwrite it. This catches the silent breakage
...
func checkStatuslineFormat(h *tmuxhost.Client) CheckResult {
	...
	// FR-2.4: a format containing atelier's freshness but no #W produces
	// a bare floating icon with no window name. Catch this regardless of
	// other segments.
	if hasFresh && !hasW {
		return CheckResult{Name: "statusline segments", Status: StatusFail,
			...
			Remediation: "set window-status-current-format to include `#W` (e.g. \"#[bold] #W #[nobold]\") and re-source so stamp-statusline can inject at the anchor"}
	}
	switch {
