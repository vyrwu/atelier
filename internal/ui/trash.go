...
func (m Model) viewTrash() string {
	...
	for i, r := range m.rows {
		b.WriteString(m.renderTrashRow(r, i == m.sel))
		b.WriteString("\n")
	}
	if m.err != "" {
		b.WriteString("\n" + lipglossWarn(m.err))
...
func (m Model) renderTrashRow(r row, selected bool) string {
	...
	when = trashTimeStyle.Render(fmt.Sprintf("%4s", when))
	right := m.spaceCounts(r.ws) // right-aligned stats column (collapsed), like Spaces
	w := m.rowWidth()
	// Budget + cap the name to match the Spaces view (the 2 is the when/name gap).
	maxName := w - visibleWidth(bar) - visibleWidth(when) - 2 - visibleWidth(right) - 4
	if maxName > 44 {
		maxName = 44
	}
	if maxName < 12 {
