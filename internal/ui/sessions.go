...
func (m Model) renderRow(r row, selected bool) string {
	...
	// The referential name (slug) leads the meta — a stable handle for the row.
	meta := r.ws.Slug
	if n := openPRs(r.ws); n > 0 {
		meta += fmt.Sprintf(" · %d PR", n)
	}
	meta += " · " + age(r.ws.Created)
	return fmt.Sprintf("%s%s %s  %s", bar, g, title, dimStyle.Render(meta))
}
